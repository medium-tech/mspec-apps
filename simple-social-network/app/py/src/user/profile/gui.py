...
from user.profile.model import Profile, field_list, longest_field_name_length
from user.profile.client import client_list_profile
...
class ProfileIndexPage(tkinter.Frame):
    ...
    def __init__(self, parent_frame, app:tkinter.Tk):
        ...
        back_button = ttk.Button(self, text='<-', command=lambda: self.app.show_frame_str('UserIndexPage'))
        back_button.grid(row=0, column=0)

        label = ttk.Label(self, text='profile', font=Fonts.heading1)
    ...
    def _list_fetch(self):
        ...
        try:
            list_response = client_list_profile(self.app.ctx, offset=self.list_offset, limit=self.list_page_size)
        ...
        for n in range(self.list_page_size):
            ...
            if item_id == '-':
                ...
                def go_to_item(_item):
                    print(f"Going to item {_item.id} in ProfileInstancePage")
                    self.app.show_frame_str('ProfileInstancePage', item=_item)
            ...
            # bio - str
            bio_text = tkinter.Text(self.table, height=1, width=20, highlightthickness=0)
            bio_text.insert(tkinter.END, getattr(item, 'bio', '-'))
            bio_text.grid(row=n + self.list_items_row_offset, column=2, padx=padx)
            ...
            # username - str
            username_text = tkinter.Text(self.table, height=1, width=20, highlightthickness=0)
            username_text.insert(tkinter.END, getattr(item, 'username', '-'))
            username_text.grid(row=n + self.list_items_row_offset, column=4, padx=padx)
...
class ProfileInstancePage(tkinter.Frame):
     
    def __init__(self, parent, controller, item=None): 
        super().__init__(parent)
        self.parent = parent
        self.controller = controller
        self.item:Profile = None
        self.item_id = '-'
    ...
    def _draw(self):
        ...
        back_button = ttk.Button(self, text='<-', command=lambda: self.controller.show_frame_str('ProfileIndexPage'))
        back_button.grid(row=0, column=0)


        label = ttk.Label(self, text=f'profile - {self.item_id}', font=Fonts.heading1)
        label.grid(row=0, column=1)
    ...
    def on_show_frame(self, item=None, **kwargs):
        self._set_item(item)
        print(f'Showing ProfileInstancePage for item: {self.item_id}')
        self._draw()