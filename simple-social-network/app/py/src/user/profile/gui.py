...
class ProfileIndexPage(tkinter.Frame):
    ...
    def _list_fetch(self):
        ...
        try:
            items = client_list_profile(self.app.ctx, offset=self.list_offset, limit=self.list_page_size)
        ...
        for n in range(self.list_page_size):
            ...
            username_text.grid(row=n + self.list_items_row_offset, column=3, padx=padx)

            # bio - str
            bio_text = tkinter.Text(self.table, height=1, width=20, highlightthickness=0)
            bio_text.insert(tkinter.END, getattr(item, 'bio', '-'))
            bio_text.grid(row=n + self.list_items_row_offset, column=4, padx=padx)
