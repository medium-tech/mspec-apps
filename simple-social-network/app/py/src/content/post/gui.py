...
from content.post.model import Post, field_list, longest_field_name_length
from content.post.client import client_list_post
...
class PostIndexPage(tkinter.Frame):
    ...
    def __init__(self, parent_frame, app:tkinter.Tk):
        ...
        label = ttk.Label(self, text='post', font=Fonts.heading1)
        ...
        for n in range(self.list_page_size):
            ...
            if item_id == '-':
                ...
                def go_to_item(_item):
                    print(f"Going to item {_item.id} in PostInstancePage")
                    self.app.show_frame_str('PostInstancePage', item=_item)
...
class PostInstancePage(tkinter.Frame):
    ...
    def _draw(self):
        ...
        back_button = ttk.Button(self, text='<-', command=lambda: self.controller.show_frame_str('PostIndexPage'))
        back_button.grid(row=0, column=0)


        label = ttk.Label(self, text=f'post - {self.item_id}', font=Fonts.heading1)
        label.grid(row=0, column=1)
    ...
    def on_show_frame(self, item=None, **kwargs):
        self._set_item(item)
        print(f'Showing PostInstancePage for item: {self.item_id}')
        self._draw()