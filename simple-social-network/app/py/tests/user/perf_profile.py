...
def perf_profile_validate(repeat:int=5, number:int=10_000) -> float:
    test_profile = Profile.example()

    return timeit.repeat(test_profile.validate, repeat=repeat, number=number)

def perf_profile_init_example(repeat:int=5, number:int=10_000) -> float:
    return timeit.repeat(Profile.example, repeat=repeat, number=number)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run performance tests for Profile validation.')
