...
def perf_post_validate(repeat:int=5, number:int=10_000) -> float:
    test_post = Post.example()

    return timeit.repeat(test_post.validate, repeat=repeat, number=number)

def perf_post_init_example(repeat:int=5, number:int=10_000) -> float:
    return timeit.repeat(Post.example, repeat=repeat, number=number)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run performance tests for Post validation.')
