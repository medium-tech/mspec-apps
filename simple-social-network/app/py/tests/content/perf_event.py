...
def perf_event_validate(repeat:int=5, number:int=10_000) -> float:
    test_event = Event.example()

    return timeit.repeat(test_event.validate, repeat=repeat, number=number)

def perf_event_init_example(repeat:int=5, number:int=10_000) -> float:
    return timeit.repeat(Event.example, repeat=repeat, number=number)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run performance tests for Event validation.')
