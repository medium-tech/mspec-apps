...
def perf_employees_validate(repeat:int=5, number:int=10_000) -> float:
    test_employees = Employees.example()

    return timeit.repeat(test_employees.validate, repeat=repeat, number=number)

def perf_employees_init_example(repeat:int=5, number:int=10_000) -> float:
    return timeit.repeat(Employees.example, repeat=repeat, number=number)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run performance tests for Employees validation.')
