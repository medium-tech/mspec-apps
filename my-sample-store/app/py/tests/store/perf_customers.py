...
def perf_customers_validate(repeat:int=5, number:int=10_000) -> float:
    test_customers = Customers.example()

    return timeit.repeat(test_customers.validate, repeat=repeat, number=number)

def perf_customers_init_example(repeat:int=5, number:int=10_000) -> float:
    return timeit.repeat(Customers.example, repeat=repeat, number=number)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run performance tests for Customers validation.')
