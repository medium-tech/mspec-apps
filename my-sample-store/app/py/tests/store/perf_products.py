...
def perf_products_validate(repeat:int=5, number:int=10_000) -> float:
    test_products = Products.example()

    return timeit.repeat(test_products.validate, repeat=repeat, number=number)

def perf_products_init_example(repeat:int=5, number:int=10_000) -> float:
    return timeit.repeat(Products.example, repeat=repeat, number=number)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run performance tests for Products validation.')
