...
from admin.employees.model import Employees, field_list, longest_field_name_length
from admin.employees.client import client_list_employees
...
class EmployeesIndexPage(tkinter.Frame):
    ...
    def __init__(self, parent_frame, app:tkinter.Tk):
        ...
        back_button = ttk.Button(self, text='<-', command=lambda: self.app.show_frame_str('AdminIndexPage'))
        back_button.grid(row=0, column=0)

        label = ttk.Label(self, text='employees', font=Fonts.heading1)
        ...
        for n in range(self.list_page_size):
            ...
            if item_id == '-':
                ...
                def go_to_item(_item):
                    print(f"Going to item {_item.id} in EmployeesInstancePage")
                    self.app.show_frame_str('EmployeesInstancePage', item=_item)
...
class EmployeesInstancePage(tkinter.Frame):
     
    def __init__(self, parent, controller, item=None): 
        super().__init__(parent)
        self.parent = parent
        self.controller = controller
        self.item:Employees = None
        self.item_id = '-'
    ...
    def _draw(self):
        ...
        back_button = ttk.Button(self, text='<-', command=lambda: self.controller.show_frame_str('EmployeesIndexPage'))
        back_button.grid(row=0, column=0)


        label = ttk.Label(self, text=f'employees - {self.item_id}', font=Fonts.heading1)
        label.grid(row=0, column=1)
    ...
    def on_show_frame(self, item=None, **kwargs):
        self._set_item(item)
        print(f'Showing EmployeesInstancePage for item: {self.item_id}')
        self._draw()