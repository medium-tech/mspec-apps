...
from store.customers.model import Customers, field_list, longest_field_name_length
from store.customers.client import client_list_customers
...
class CustomersIndexPage(tkinter.Frame):
    ...
    def __init__(self, parent_frame, app:tkinter.Tk):
        ...
        label = ttk.Label(self, text='customers', font=Fonts.heading1)
    ...
    def _list_fetch(self):
        ...
        try:
            list_response = client_list_customers(self.app.ctx, offset=self.list_offset, limit=self.list_page_size)
        ...
        for n in range(self.list_page_size):
            ...
            if item_id == '-':
                ...
                def go_to_item(_item):
                    print(f"Going to item {_item.id} in CustomersInstancePage")
                    self.app.show_frame_str('CustomersInstancePage', item=_item)
            ...
            # customer_name - str
            customer_name_text = tkinter.Text(self.table, height=1, width=20, highlightthickness=0)
            customer_name_text.insert(tkinter.END, getattr(item, 'customer_name', '-'))
            customer_name_text.grid(row=n + self.list_items_row_offset, column=2, padx=padx)
            ...
            email_text.grid(row=n + self.list_items_row_offset, column=3, padx=padx)
            ...
            phone_number_text.grid(row=n + self.list_items_row_offset, column=4, padx=padx)
...
class CustomersInstancePage(tkinter.Frame):
     
    def __init__(self, parent, controller, item=None): 
        super().__init__(parent)
        self.parent = parent
        self.controller = controller
        self.item:Customers = None
        self.item_id = '-'
    ...
    def _draw(self):
        ...
        back_button = ttk.Button(self, text='<-', command=lambda: self.controller.show_frame_str('CustomersIndexPage'))
        back_button.grid(row=0, column=0)


        label = ttk.Label(self, text=f'customers - {self.item_id}', font=Fonts.heading1)
        label.grid(row=0, column=1)
    ...
    def on_show_frame(self, item=None, **kwargs):
        self._set_item(item)
        print(f'Showing CustomersInstancePage for item: {self.item_id}')
        self._draw()