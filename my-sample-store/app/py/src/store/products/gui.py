...
from store.products.model import Products, field_list, longest_field_name_length
from store.products.client import client_list_products
...
class ProductsIndexPage(tkinter.Frame):
    ...
    def __init__(self, parent_frame, app:tkinter.Tk):
        ...
        back_button = ttk.Button(self, text='<-', command=lambda: self.app.show_frame_str('StoreIndexPage'))
        back_button.grid(row=0, column=0)

        label = ttk.Label(self, text='products', font=Fonts.heading1)
    ...
    def _list_fetch(self):
        ...
        try:
            list_response = client_list_products(self.app.ctx, offset=self.list_offset, limit=self.list_page_size)
        ...
        for n in range(self.list_page_size):
            ...
            if item_id == '-':
                ...
                def go_to_item(_item):
                    print(f"Going to item {_item.id} in ProductsInstancePage")
                    self.app.show_frame_str('ProductsInstancePage', item=_item)
            ...
            # in_stock - bool
            in_stock_text = tkinter.Text(self.table, height=1, width=5, highlightthickness=0)
            in_stock_text.insert(tkinter.END, str(getattr(item, 'in_stock', '-')).lower())
            in_stock_text.grid(row=n + self.list_items_row_offset, column=2, padx=padx)

            
            # price - float
            price_text = tkinter.Text(self.table, height=1, width=10, highlightthickness=0)
            price_text.insert(tkinter.END, str(getattr(item, 'price', '-')))
            price_text.grid(row=n + self.list_items_row_offset, column=3, padx=padx)

            
            # product_name - str
            product_name_text = tkinter.Text(self.table, height=1, width=20, highlightthickness=0)
            product_name_text.insert(tkinter.END, getattr(item, 'product_name', '-'))
            product_name_text.grid(row=n + self.list_items_row_offset, column=4, padx=padx)
...
class ProductsInstancePage(tkinter.Frame):
     
    def __init__(self, parent, controller, item=None): 
        super().__init__(parent)
        self.parent = parent
        self.controller = controller
        self.item:Products = None
        self.item_id = '-'
    ...
    def _draw(self):
        ...
        back_button = ttk.Button(self, text='<-', command=lambda: self.controller.show_frame_str('ProductsIndexPage'))
        back_button.grid(row=0, column=0)


        label = ttk.Label(self, text=f'products - {self.item_id}', font=Fonts.heading1)
        label.grid(row=0, column=1)
    ...
    def on_show_frame(self, item=None, **kwargs):
        self._set_item(item)
        print(f'Showing ProductsInstancePage for item: {self.item_id}')
        self._draw()