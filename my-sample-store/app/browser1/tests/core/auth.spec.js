...
test.describe('Authentication Workflow', () => {
  ...
  test('should complete full user workflow: create user, login, logout', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText('Create User - my_sample_store');
    ...
    await expect(page.locator('h1')).toContainText('Login - my_sample_store');
    ...
    await page.waitForURL('http://localhost:7007/');
    ...
    await expect(page.locator('h1')).toContainText('User Account - my_sample_store');
  ...
  test('should handle login with invalid credentials', async ({ page }) => {
    await page.goto('http://localhost:7007/auth/login.html');
  ...
  test('should validate password confirmation on create user', async ({ page }) => {
    await page.goto('http://localhost:7007/auth/create-user.html');
